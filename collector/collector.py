    ...
    for pid,name,issuer in rows:
        ...
        for r in c.execute("SELECT platform,reward_text,reward_won,period_end,source_url FROM event WHERE card_product_id=? AND status='active' AND substr(last_seen,1,7)=? ORDER BY reward_won DESC",(pid,current_month_kst())).fetchall():
            ...
            if ov: mw,bw=ov["main"],ov["bonus"]
                ...
                _cm=CG_MAIN_NK.get(_nk(name),0); _cb=CG_BONUS_NK.get(_nk(name),0); _tt=(r[2] or 0)
                if 0<_cm<_tt:                        # 주요가 전체 미만일 때만(전체>주요 → 부가 존재)
                    mw=_cm; bw=_tt-_cm
...
CG_BONUS_NK={} # _nk(카드명) → 부가(원)  주요가 없을 때(mainWon=None) 부가 수치로 분해(부가=bonusWon, 주요=전체-부가)
...
if __name__=="__main__":
    ...
    try:                                          # 카드고릴라 주요 수치 → CG_MAIN_NK (export_json에서 부가=전체-주요)
        ...
        print(f"cardgorilla 주요 수치 적재: {len(CG_MAIN_NK)}건 / 부가 {len(CG_BONUS_NK)}건")
