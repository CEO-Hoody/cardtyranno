...
function splTier(e,mt){var t=(e.reward_won||0),b=(e.bonus_won||0),m=(e.main_won!=null?e.main_won:null);
 if(b>0){if(m==null)m=Math.max(t-b,0);else{if(m>t)m=t;b=Math.max(t-m,0);}return {t:t,m:m,b:b};}
 if(m!=null&&m<t)return {t:t,m:m,b:Math.max(t-m,0)};
 mt=mt||0;if(mt>0&&mt<t)return {t:t,m:mt,b:Math.max(t-mt,0)};
 return {t:t,m:(m==null?t:Math.min(m,t)),b:0};}
...
Promise.all([
 ...
 Promise.all(months.map(function(m){return m===_cm?Promise.resolve(null):fetch('history/'+m+'.json').then(function(r){return r.json();}).catch(function(){return null;});})).then(function(MS){
  ...
  function renderDyn(i){var g=group[i],p=g.p,e=g.e;var S=buildSeries(p,e);
   ...
   var _pmt=(CARDMAP[_nk(p.name)]||{}).main_tier;_pmt=_pmt&&_pmt.reward;
   function _mainOf(x){return splTier(x,_pmt).m;}
