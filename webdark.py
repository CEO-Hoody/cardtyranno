...
function splTier(e,mt){var t=(e.reward_won||0);mt=mt||0;
 ...
 // 티어 없으면 플랫폼 분해로 폴백(분해됨=그대로, 미분해=전액 주요)
 var b=(e.bonus_won||0),pm=(e.main_won!=null?e.main_won:null);
 if(b>0){if(pm==null)pm=Math.max(t-b,0);else{if(pm>t)pm=t;b=Math.max(t-pm,0);}return {t:t,m:pm,b:b};}
 if(pm!=null&&pm<t)return {t:t,m:pm,b:Math.max(t-pm,0)};
 return {t:t,m:(pm==null?t:Math.min(pm,t)),b:0};}
...
ISSUE_BODY=('<style>'
 ...
 '.pcmp-banners{display:grid;grid-template-columns:1fr;gap:14px;margin-top:18px}'
...
Promise.all([fetch('platform_events.json').then(r=>r.json()),fetch('cards.json').then(r=>r.json()).catch(function(){return {cards:{}};})]).then(function(A){
 ...
 (function(){var tmp=[],mx={};
  prods.forEach(function(p){var iss=p.issuer||'기타';(p.events||[]).forEach(function(e){var _pp=(p.platforms||{})[e.platform]||{};var u=_best(e.platform,e.url||_pp.url,_pp.id);if(e.platform==='cardgorilla'){var _cg=_cgUrl(iss,_pp.id);if(_cg)u=_cg;}
   var _s=splTier(e,MTIER[_nk2(p.name)]);
   tmp.push({issuer:iss,card:p.name,platform:(PN[e.platform]||e.platform),pk:e.platform,benefit:_rwd(e.reward_text,e.reward_won),url:u||('carddetail.html?n='+encodeURIComponent(p.name)),period:(e.period_end?('~'+String(e.period_end).slice(5).replace('-','/')):''),pe:(e.period_end||''),won:_s.t,main:_s.m,bonus:_s.b,text:(e.reward_text||''),img:(p.img||IMG[_nk2(p.name)]||'')});
 ...
 var TBL=prods.filter(function(p){return new Set((p.events||[]).map(e=>e.platform)).size>1;}).map(function(p){
  var _mt=MTIER[_nk2(p.name)];var o={};(p.events||[]).forEach(function(e){var w=e.reward_won||0;var c=o[e.platform];if(!c||w>c.t)o[e.platform]=splTier(e,_mt);});
...
Promise.all([
 ...
 var col=PBC[P]||'#888',pnm=(P==='issuer'?((anchor.p.issuer||'카드사')+' 공식'):(PNM[P]||P)),N=group.length;
 ...
 Promise.all(months.map(function(m){return m===_cm?Promise.resolve(null):fetch('history/'+m+'.json').then(function(r){return r.json();}).catch(function(){return null;});})).then(function(MS){
  ...
  function renderDyn(i){var g=group[i],p=g.p,e=g.e;var S=buildSeries(p,e);
   ...
   // 코칭은 '플랫폼 간 주요혜택 우열' 비교가 목적 → 플랫폼별 main_won 기준 유지(표시용 splTier와 별개)
   ...
   var coach='<div class="rg-coach"><div class="rg-coach-badge"><span class="rg-coach-ring '+recCls+'">'+recSym+'</span><div class="lab rg-only-pc" style="color:'+(rec==='rec'?'var(--success)':(rec==='hold'?'#cf9220':'rgba(0,0,0,.55)'))+'">'+recLab+'</div></div>'
